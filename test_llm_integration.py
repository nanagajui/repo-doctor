...
from repo_doctor.utils.llm import LLMAnalyzer, LLMClient, LLMFactory
    ...
    for url in test_urls:
        ...
        if await client._check_availability():
            ...
            response = await client.generate_completion(
                "What is Python? Answer in one sentence.", max_tokens=50
            )
...
async def test_llm_analyzer():
    ...
    if doc_analysis:
        ...
        print(
            f"   - Complexity: {doc_analysis.get('installation_complexity', 'Unknown')}"
        )
...
async def test_integration_workflow():
    ...
    try:
        ...
        print(
            f"   - Analysis Agent LLM: {'✅' if analysis_agent.llm_analyzer else '❌'}"
        )
        print(
            f"   - Resolution Agent LLM: {'✅' if resolution_agent.llm_analyzer else '❌'}"
        )
