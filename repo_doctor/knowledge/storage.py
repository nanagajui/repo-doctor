...
class FileSystemStorage:
    ...
    def store_analysis(self, repo_key: str, analysis: Analysis, commit_hash: str) -> bool:
    ...
    def store_solution(self, repo_key: str, solution: Resolution, outcome: ValidationResult) -> bool:
    ...
    def update_compatibility_matrix(self, matrix_type: str, data: Dict[str, Any]) -> bool:
    ...
    def get_cached_analysis(self, repo_key: str, cache_key: str) -> Optional[Dict[str, Any]]:
        """Get cached analysis result."""
        cache_file = self.base_path / "cache" / f"{repo_key.replace('/', '_')}_{cache_key}.json"
    ...
    def cache_analysis(self, repo_key: str, cache_key: str, data: Dict[str, Any]) -> bool:
                ...
                for repo_dir in repos_dir.iterdir():
                    if repo_dir.is_dir():
                        ...
                        if analyses_dir.exists():
                            stats["total_analyses"] += len(list(analyses_dir.glob("*.json")))
                        ...
                        if solutions_dir.exists():
                            ...
                            if successful_dir.exists():
                                stats["successful_solutions"] += len(list(successful_dir.glob("*.json")))
                            if failed_dir.exists():
                                stats["failed_solutions"] += len(list(failed_dir.glob("*.json")))
