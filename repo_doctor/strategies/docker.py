...
class DockerStrategy(BaseStrategy):
    ...
    def _generate_dockerfile(self, analysis: Analysis) -> str:
        ...
        # Safely collect python dependencies; tests may provide Mock or non-iterable
        python_deps: List[str] = []
        try:
            deps_iter = analysis.get_python_dependencies()
            # Basic duck-typing: attempt iteration
            for dep in deps_iter:  # type: ignore
                name = getattr(dep, "name", None)
                if isinstance(name, str) and name:
                    python_deps.append(name)
        except Exception:
            # Fall back to empty list if dependencies are not available/iterable
            python_deps = []
