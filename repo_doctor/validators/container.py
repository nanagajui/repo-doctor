...
from ..models.resolution import ValidationResult, ValidationStatus, Resolution
...
class ContainerValidator:
    ...
    def validate_resolution(self, resolution: Resolution, analysis: Analysis, 
                          timeout: int = 300) -> ValidationResult:
    ...
    def _validate_in_directory(self, directory: Path, analysis: Analysis, 
        ...
        try:
            ...
            for test_name, test_command in validation_tests.items():
                ...
                gpu_args = {}
                if analysis.is_gpu_required() and self.gpu_available and "gpu" in test_name.lower():
                    gpu_args = {"device_requests": [
                ...
                if result['StatusCode'] == 0:
                    ...
                    logs.append(f"❌ {test_name}: FAILED (exit code {result['StatusCode']})")
    ...
    def _get_validation_tests(self, analysis: Analysis) -> Dict[str, str]:
        ...
        for dep in python_deps[:5]:  # Test first 5 dependencies
            if dep.name in ['torch', 'tensorflow', 'jax']:
                tests[f"import_{dep.name}"] = f"python -c 'import {dep.name}; print(f\"{dep.name} imported successfully\")'"
        ...
        if analysis.is_gpu_required():
            ...
            if any(dep.name == 'torch' for dep in python_deps):
                tests["pytorch_gpu"] = "python -c 'import torch; print(f\"CUDA available: {torch.cuda.is_available()}\")'"
            
            if any(dep.name in ['tensorflow', 'tensorflow-gpu'] for dep in python_deps):
                tests["tensorflow_gpu"] = "python -c 'import tensorflow as tf; print(f\"GPU devices: {len(tf.config.list_physical_devices(\"GPU\"))}\")'"
